...
import {
    ...
    setHistory,
    updateReaction,
} from "../features/api/messenger/messagesSlice";
import {
    setMessageRead,
    setUserTyping,
    setActiveChat,
...
export function useSocket() {
    ...
    const emitJoinChat = (chatId: number) => {
        socket.emit('join_chat', { chatId });
    };

    const emitLeaveChat = (chatId: number) => {
        socket.emit('leave_chat', { chatId });
    };

    const emitSendMessage = (chatId: number, text: string, replyToId?: number) => {
        socket.emit('message_send', { chatId, text, replyToId });
    };

    const emitEditMessage = (messageId: number, text: string) => {
        socket.emit('message_edit', { messageId, text });
    };

    const emitDeleteMessage = (messageId: number) => {
        socket.emit('message_delete', { messageId });
    };

    const emitSendTyping = (chatId: number, isTyping: boolean) => {
        socket.emit('typing', { chatId, isTyping });
    };

    const emitReactMessage = (messageId: number, emoji: string) => {
        socket.emit('message_react', { messageId, emoji });
    };

    const emitReadMessage = (chatId: number, messageId: number) => {
        socket.emit('message_read', { chatId, messageId });
    };

    const emitToggleHidden = (hidden: boolean) => {
        socket.emit('presence:hidden', hidden)
    };

    const emitPresenceState = (status: 'online' | 'offline') => {
